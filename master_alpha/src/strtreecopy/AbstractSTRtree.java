...
public abstract class AbstractSTRtree implements Serializable {
  ...
  private AbstractNode tempNode = null;			//used to save node where last item was removed to add new item
  ...
  private boolean removeItem(AbstractNode node, Object item)
  {
    ...
    if (childToRemove != null) {
      if(replace){
    	 tempNode =  node;    //save node (reference?)
      }
  ...
  private boolean remove(Object searchBounds, AbstractNode node, Object item) {
    ...
    if (replace) return found; //dont delete child because new one will be added
  ...
  public boolean replace(Object searchBounds, Object itemReplace, Object boundsNew ,Object itemNew){
	 replace=true; 
	 //delete item
	 boolean done =  remove(searchBounds,  itemReplace);
	 //add new Item
	 ItemBoundable newChild =new ItemBoundable(boundsNew,itemNew);
	 //tempNode.getChildBoundables().add(newChild);
	 tempNode.addChildBoundable(newChild);
	 Collections.sort(tempNode.getChildBoundables(), getComparator());
